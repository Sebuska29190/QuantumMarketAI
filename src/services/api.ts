...
// We are now calling our proxy server instead of external APIs
...
const handleRateLimit = async (apiName: 'newsdata', crypto: string) => {
  const limiter = rateLimiter[apiName];
  const now = Date.now();
...
export const api = {
  async getPrice(crypto: string): Promise<CryptoPrice> {
    ...
    try {
      // Use the new proxy endpoint for price data
      const response = await axios.get(`${PROXY_API}/coingecko/simple-price`, {
        params: {
          ids: crypto,
          vs_currencies: 'usd',
  ...
  async getHistoricalData(crypto: string, days: number = 200) {
    ...
    try {
      // Use the new proxy endpoint for historical data
      const response = await axios.get(`${PROXY_API}/coingecko/market-chart`, {
        params: {
          id: crypto,
          vs_currency: 'usd',
          days: days.toString(),
  ...
  async getNews(crypto: string, limit: number = 10): Promise<{ news: NewsItem[] }> {
    ...
    try {
      ...
      // Use the new proxy endpoint for news data
      const response = await axios.get(`${PROXY_API}/news`, {
        params: {
  ...
  async getVolume(crypto: string, days: number = 30) {
    ...
    try {
      // Use the new proxy endpoint for market chart
      const response = await axios.get(`${PROXY_API}/coingecko/market-chart`, {
        params: {
          id: crypto,
          vs_currency: 'usd',
  ...
  async getBatchPrices(coins: string[]): Promise<BatchPriceData> {
    ...
    try {
      // Use the new proxy endpoint for simple price
      const response = await axios.get(`${PROXY_API}/coingecko/simple-price`, {
        params: {
          ids: coins.join(','),
          vs_currencies: 'usd',
  ...
  async getBatchHistoricalData(coins: string[], days: number = 200) {
    ...
    try {
      const promises = coins.map(coin => 
        // Use the new proxy endpoint for historical data
        axios.get(`${PROXY_API}/coingecko/market-chart`, {
          params: {
            id: coin,
            vs_currency: 'usd',
            days,
  ...
  // This function can be removed as the proxy server will handle the rate limiting
